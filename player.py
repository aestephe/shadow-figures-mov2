...
bar_lengths = itertools.cycle(	[ 3.0,    3.0,    3.0,    3.0,    3.0,    3.0,    3.0])      
macro_ratios = itertools.cycle(	[[2, 1], [2, 3], [1, 1], [2, 3], [1, 1], [1, 2], [1, 2]])
micro_ratios = itertools.cycle(	[[1, 1], [2, 1], [1, 1], [2, 1], [1, 1], [1, 1], [1, 1]])
...
for n in note_list:
	if n.midi_number == 0:
		...
		if n.midi_number.is_integer():
			...
			FUND = n.fundamental
			inst2.play_chord([math.ceil(n.midi_number+7)], length = bar_length*(macro_ratio[1]/(macro_ratio[0]+macro_ratio[1]))*(micro_ratio[1]/(micro_ratio[0]+micro_ratio[1])), volume = 1)
			inst2.play_chord([math.ceil(n.midi_number)], length = bar_length*(macro_ratio[1]/(macro_ratio[0]+macro_ratio[1]))*(micro_ratio[0]/(micro_ratio[0]+micro_ratio[1])), volume = 1)
			played_b = True
...
s.new_osc_part("echo_off", 8000, "127.0.0.1").play_note(0, 0.0, 0.01, blocking = False)
